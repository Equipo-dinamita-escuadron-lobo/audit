package com.audit.infrastructure.adapters.input.messageBroker.aspect;
 ...
 * 
 *        This advice intercepts methods annotated with @RabbitListener to
 *        extract
 *        a JWT token from message headers, decode the tenantId, set it in the
 *        TenantContext, execute the listener method, and finally clear the
 *        context.
 */
@Aspect
@Component
public class JWTContextRabbitMqAspect {
    ...
    public Object setTenantContext(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (message == null) {
            logger.warn("Listener [{}] does not receive a 'Message' object. Cannot set tenant context.",
        ...
        if (jwtToken == null || jwtToken.isEmpty()) {
            logger.error("Message received without the '{}' header. Processing without tenant context.",
                    JWT_TOKEN_HEADER);
            return joinPoint.proceed(); // Execute without context
        }
