...
import com.audit.application.usecases.queries.export.DocumentExportUseCaseImpl;
...
class DocumentExportUseCaseImplTest {

    @Mock
    private ExportJobTracker jobTracker;

    @Mock
    private IDocumentAsyncExportPort asyncExportPort;

    @InjectMocks
    private DocumentExportUseCaseImpl useCase;

    @Test
    @DisplayName("execute - debe crear job de exportación documental y lanzar proceso asíncrono")
    void execute_validRequest_shouldCreateJobAndProcess() {
        ...
        when(jobTracker.createJob(eq("ENT-001"), anyString(), eq(ExportFormat.EXCEL), eq(ExportType.DOCUMENT)))
                .thenReturn("JOB-001");
        ...
        verify(jobTracker).createJob(
                ...
                eq(ExportType.DOCUMENT));

        assertTrue(fileNameCaptor.getValue().startsWith("documents_audit_"));
